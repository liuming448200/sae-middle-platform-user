...
class MysqlClient {
  ...
  private function __construct() {
    SaeLog::writelog_debug("Create MysqlClient not allowed");
    // Logg::Warn("Create MysqlClient not allowed");
  }
  ...
  private static function GetMasterClient($db_name, $db_index=0) {
    ...
    if (empty(self::$master_config_[$db_key])) {
      SaeLog::writelog_debug("Invalid master db name:${db_name}, index:$db_key");
      // Logg::Warn("Invalid master db name:${db_name}, index:$db_key");
      return NULL;
    }
    ...
    if ($client->connect_errno) {
      SaeLog::writelog_debug("Failed to connect to MySQL: (" . $client->connect_errno . ") "
                . $client->connect_error . ${db_name} . " idx:" . $db_key);
      // Logg::Warn("Failed to connect to MySQL: (" . $client->connect_errno . ") "
      //           . $client->connect_error . ${db_name} . " idx:" . $db_key);
      return NULL;
    }
  ...
  private static function GetSlaveClient($db_name, $db_index=0) {
    ...
    if (empty(self::$slave_config_[$db_key])) {
      SaeLog::writelog_debug("Invalid slave db name:${db_name}" . " idx:" . $db_key);
      // Logg::Warn("Invalid slave db name:${db_name}" . " idx:" . $db_key);
      return NULL;
    }
  ...
  public static function ExecuteUpdate($db_name, $sql, $db_index=0) {
    ...
    if (!$client)
    {
      SaeLog::writelog_debug('ExecuteUpdate :' . $sql . '; GetMasterClient failed; Cost time: ' . $runTimeUtil->spent() . 'ms;');
      // Logg::Warn('ExecuteUpdate :' . $sql . '; GetMasterClient failed; Cost time: ' . $runTimeUtil->spent() . 'ms;');
      return false;
    }
    ...
    if ($client->errno) {
      ...
      SaeLog::writelog_debug("ExecuteUpdate failed:" . $client->errno . "," . $client->error . '; ExecuteUpdate :' . $sql . '; Cost time: ' . $runTimeUtil->spent() . 'ms;');
      // Logg::Warn("ExecuteUpdate failed:" . $client->errno . "," . $client->error . '; ExecuteUpdate :' . $sql . '; Cost time: ' . $runTimeUtil->spent() . 'ms;');
      return false;
    }
    // Logg::Info("ExecuteUpdate success:" . $sql . ';This sql cost time: ' . $runTimeUtil->spent() . 'ms;');
    return true;
  }
  ...
  public static function ExecuteQuery($db_name, $sql, $db_index=0, $assoc=1, $slave=1) {
    ...
    if (!$client)
    {
      SaeLog::writelog_debug('ExecuteQuery:' . $sql . ';Get client failed; Cost time: ' . $runTimeUtil->spent() . 'ms;');
      // Logg::Warn('ExecuteQuery:' . $sql . ';Get client failed; Cost time: ' . $runTimeUtil->spent() . 'ms;');
      return false;
    }
    $result = $client->query($sql);
    if ($client->errno) {
      ...
      SaeLog::writelog_debug("ExecuteQuery failed:".$client->errno.",".$client->error . '; ExecuteQuery:' . $sql . '; Cost time: ' . $runTimeUtil->spent() . 'ms;');
      // Logg::Warn("ExecuteQuery failed:".$client->errno.",".$client->error . '; ExecuteQuery:' . $sql . '; Cost time: ' . $runTimeUtil->spent() . 'ms;');
      return false;
    }
    if (!($result) || 0 == $result->num_rows)
    {
      // Logg::Info('ExecuteQuery:' . $sql . ';This sql cost time: ' . $runTimeUtil->spent() . 'ms;');
      return array();
    }
    $result_array = array();
    if ($assoc) {
